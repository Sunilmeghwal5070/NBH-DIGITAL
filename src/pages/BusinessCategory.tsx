...
export default function BusinessCategory() {
  ...
  useEffect(() => {
    if (id === 'home-services') return;

    let q = query(collection(db, 'businesses'));
    if (id && id !== 'all') {
      q = query(collection(db, 'businesses'), where('category', '==', id));
    }
    
    // Some fallback mappings for electrician vs generic home services in mock
    if (id === 'electrician' || id === 'ac_repair') {
      q = query(collection(db, 'businesses'), where('category', '==', id));
    }
           ...
           >
              {homeServicesList.map((service, i) => (
                 <motion.div 
                    ...
                    onClick={() => navigate(`/business/category/${service.id}`)}
                 >
  ...
  const getServiceName = (catId: string) => {
     return homeServicesList.find(s => s.id === catId)?.name || category?.name || catId;
  };
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-4 flex items-center bg-white sticky top-0 z-10 justify-between shadow-sm">
         ...
         <h1 className="text-xl font-bold text-gray-900 text-center flex-grow capitalize">{id ? getServiceName(id) : "Category"}</h1>
      ...
      <div className="p-4 mt-2">
        ...
        <div className="space-y-4">
           {loading ? (
              <div className="flex justify-center py-10"><div className="w-8 h-8 rounded-full border-4 border-gray-200 border-t-purple-600 animate-spin"></div></div>
               ...
               >
                   <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 mr-4 bg-gray-50 flex items-center justify-center border border-gray-100">
                      {biz.isPerson ? (
                          ...
                          biz.images && biz.images.length > 0 ? (
                            <img src={biz.images[0]} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                          ) : (
                   ...
                   <div className="flex flex-col justify-center flex-grow">
                      ...
                      <p className="text-xs font-medium text-gray-500 uppercase tracking-widest leading-tight mb-2">{biz.location || biz.category} {biz.distance && `• ${biz.distance} km`}</p>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="border-2 border-dashed border-gray-200 rounded-2xl py-12 text-center text-gray-400 font-medium text-sm mt-8">
                 No listings found in this category. Be the first to register!
              </motion.div>
           )}
        </div>
      </div>
